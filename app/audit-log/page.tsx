              ...
              <div className="mx-auto max-w-6xl">
                <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-2 text-on-surface-variant">Chronological record of investigation actions recorded for the active case session.</p>
                  </div>
                  <input
                    value={query}
                ...
                <div className="overflow-hidden rounded-xl border border-outline-variant bg-surface-container">
                  ...
                  {entries.length ? entries.map(entry=>(
                    <div key={entry.id} className="grid grid-cols-[150px_1fr_140px] gap-4 border-b border-outline-variant/40 p-4 text-sm hover:bg-surface-container-low">
                      <span className="font-code-sm text-primary">{entry.time}</span>
                      <span>{entry.message}</span>
                      <span className="font-code-sm text-outline">{entry.actor}</span>
                    </div>
                  )) : (
                    <div className="p-12 text-center text-sm text-outline">No audit events match this search.</div>
                  )}
                </div>
              </div>
            </div>
